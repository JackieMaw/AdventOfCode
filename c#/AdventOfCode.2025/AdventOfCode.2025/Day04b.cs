...
public class Day04b
{
    ...
    {
        var totalPaperRollsCollected = 0L;

        var (numberOfPaperRollsCollectedOnThisRound, newInput) = CollectPaperRolls(input);
        totalPaperRollsCollected += numberOfPaperRollsCollectedOnThisRound;
        Console.WriteLine($"Collected: {numberOfPaperRollsCollectedOnThisRound} TOTAL: {totalPaperRollsCollected}");

        while (numberOfPaperRollsCollectedOnThisRound > 0)
        {
            (numberOfPaperRollsCollectedOnThisRound, newInput) = CollectPaperRolls(newInput);            
            totalPaperRollsCollected += numberOfPaperRollsCollectedOnThisRound;
        }

        return totalPaperRollsCollected;
    }


    private (long, string[]) CollectPaperRolls(string[] input)
    {
        ...
        return (howManyPaperRollsCollected, newInput);
    }
