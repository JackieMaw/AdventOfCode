...
def get_stars_count(leaderboard_json, goal) -> int:
    ...
    try:
        ...
        return -1
    return (total_stars, count_goal)

def process_leaderboard(leaderboard, code):
    leaderboard_json = get_leaderboard_json(YEAR, code)
    (total_stars, count_goal) = get_stars_count(leaderboard_json, goal)
    return (leaderboard, total_stars, count_goal)

def sort_by_total_stars(stat):
    (_, total_stars, _) = stat
    return total_stars
...
for (leaderboard, total_stars, count_goal) in stats:
    print(f"{leaderboard} leaderboard has {total_stars} stars, with {count_goal} participants achieving at least {goal} stars")