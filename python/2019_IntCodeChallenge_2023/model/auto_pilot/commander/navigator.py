...
class Navigator(Commander): 
    ...
    def _get_shortest_path(self, start_room, end_room):
        ...
        shortest_path[start_room.name] = (0, None, None) # room_name = (cost, previous_room_name, door_to_get_to_room)

        while len(priority_queue) > 0:
            (distance_to_current_room, current_room_name) = heapq.heappop(priority_queue) # this pops the next closest node off the queue
            ...
            for (door, neighbouring_room) in current_room.doors.items():
                ...
                if neighbouring_room.name not in visited_rooms: # we have never visited this room before
                    ...
                    if neighbouring_room.name not in shortest_path:
                        shortest_path[neighbouring_room.name] = (new_distance_to_neighbouring_room, current_room.name, door)
                    ...
                    if not neighbouring_room.name in visited_rooms:
                        heapq.heappush(priority_queue, (new_distance_to_neighbouring_room, neighbouring_room.name))
        ...
        while current_room_name is not start_room.name:
             (distance, previous_room_name, direction_from_previous_room) = shortest_path[current_room_name]
